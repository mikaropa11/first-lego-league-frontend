...
export default function EditVolunteerModal({ volunteer, updateAction }: Readonly<Props>) {
    ...
    const [formData, setFormData] = useState(() => {
        ...
        const foundCountry = COUNTRY_CODES.find((country) => fullPhone.startsWith(country.code));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(result.error || t.volunteers.failedToUpdate);
        } catch (err) {
            setError(err instanceof Error ? err.message : t.volunteers.failedToUpdate);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 text-zinc-900 dark:text-zinc-100">
            <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
                <h2 className="text-xl font-bold mb-6 text-center">{t.volunteers.editVolunteerTitle}</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="space-y-1">
                        <label htmlFor="volunteer-name" className="text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 ml-1">
                        ...
                        <div className="pt-2">
                            <label className="flex items-center space-x-3 cursor-pointer p-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
                                ...
                                <span className="text-sm font-medium">
                                    {t.volunteers.isExpertRole.replace('{role}', volunteer.type)}
                                </span>
                            </label>
                        </div>
                    )}
                    ...
                    <div className="mt-8 flex justify-end space-x-3 border-t border-zinc-200 dark:border-zinc-800 pt-5">
                        ...
                        >
                            {loading ? t.volunteers.saving : t.volunteers.saveChanges}
                        </button>
                    </div>
                </form>
            </div>
